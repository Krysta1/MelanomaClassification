...
def test(best_roc, fold, meta_features, device, train_df, val_idx, test_df, oof):
    best_model_path = model_dir + [file for file in os.listdir(model_dir) if
...
def main():
        ...
        if fold == 1:
            ...
            for epoch in range(epochs):
                ...
                if val_roc > best_roc:
                    ...
                    torch.save(model.state_dict(),
                               model_dir + f"{args.version}_Fold{fold + 1}_Epoch{epoch + 1}_ValidAcc_{val_acc:.3f}_ROC_{val_roc:.3f}.pth")
                else:
    ...
    pd.Series(oof.reshape(-1,)).to_csv(f'oof_{args.version}.csv', index=False)
